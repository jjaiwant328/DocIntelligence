...
app = FastAPI(
    title="DocIntelligence — QSR Supply Chain AI",
    description="AI-powered document intelligence for QSR supply chain operations. "
                "Combines ai_parse_document, ontology mapping, vector search, and AI agents.",
    version="2.0.0",
)

# Track module load status for the /_health endpoint
_docintel_load_error: str | None = None
_platform_load_error: str | None = None

# Register DocIntelligence extended routes
try:
    from docintel_routes import router as docintel_router
    app.include_router(docintel_router)
    print("✅ DocIntelligence routes registered at /api/docintel/*")
except Exception as _e:
    import traceback as _tb
    _docintel_load_error = f"{type(_e).__name__}: {_e}\n{_tb.format_exc()}"
    print(f"⚠️ DocIntelligence routes not loaded: {_docintel_load_error}")

# Register Platform multi-domain routes
try:
    from platform_routes import router as platform_router
    app.include_router(platform_router)
    print("✅ Platform routes registered at /api/platform/*")
except Exception as _e:
    import traceback as _tb
...
@app.get("/_health")
def health_check():
    """Diagnostic endpoint — returns module load status and Python version."""
    import sys, platform as _pl
    return {
        "status": "ok",
        "python": sys.version,
        "platform": _pl.platform(),
        "docintel_routes": "loaded" if _docintel_load_error is None else "FAILED",
        "docintel_error": _docintel_load_error,
        "platform_routes": "loaded" if _platform_load_error is None else "FAILED",
        "platform_error": _platform_load_error,
    }
...
# Resolve in priority order: env var → app.yaml value → sibling "static" dir
_static_from_env = os.getenv("STATIC_FILES_PATH", YAML_CONFIG.get("STATIC_FILES_PATH", ""))
if _static_from_env and os.path.exists(_static_from_env):
    target_dir = _static_from_env
else:
    target_dir = "static"
...
# ── Fallback direct routes for UC volume endpoints (guaranteed to work even if
#    docintel_router failed to load — routes are matched before the catch-all) ──

@app.get("/api/docintel/list-volumes")
async def app_list_uc_volumes(catalog_filter: str = ""):
    """Forward to docintel handler or inline fallback."""
    try:
        from docintel_routes import list_uc_volumes
        return await list_uc_volumes(catalog_filter=catalog_filter)
    except ImportError:
        pass
    ...
    try:
        ...
        try:
            resp = w.statement_execution.execute_statement(
                warehouse_id=WH_ID,
                statement=f"SHOW VOLUMES IN {CATALOG}",
                wait_timeout="20s",
            )
            cols = [c.name for c in (resp.manifest.schema.columns or [])]
            for row in (resp.result.data_array or []):
                d = dict(zip(cols, row))
                cat = d.get("catalog") or CATALOG
                sch = d.get("database") or d.get("schema_name") or "raw"
                vn  = d.get("volume_name") or d.get("name") or ""
                if vn:
                    results.append({"label": f"{cat}.{sch}.{vn}", "full_path": f"/Volumes/{cat}/{sch}/{vn}"})
        except Exception as e:
            return {"volumes": [], "total": 0, "method": "fallback", "debug_error": str(e)}
        return {"volumes": results, "total": len(results), "method": "inline-fallback"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))


@app.get("/api/docintel/volume-files")
async def app_list_volume_files(domain_id: str = "supply_chain", volume_path: str = ""):
    """Forward to docintel handler or inline fallback."""
    try:
        from docintel_routes import list_volume_files
        return await list_volume_files(domain_id=domain_id, volume_path=volume_path)
    except ImportError:
        pass
    # Inline minimal fallback
    try:
        from databricks.sdk import WorkspaceClient
        CATALOG = os.getenv("DOCINTEL_CATALOG", "jai_docintel")
        vpath = volume_path.rstrip("/") if volume_path else f"/Volumes/{CATALOG}/raw/documents"
        w = WorkspaceClient()
        files = []
        try:
            ...
            return {"domain_id": domain_id, "volume_path": vpath, "files": [], "message": str(e)}
        return {"domain_id": domain_id, "volume_path": vpath, "files": files, "total": len(files)}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
