...
@router.post("/classify-extract")
async def classify_and_extract(req: ClassifyExtractRequest, domain_id: str = "supply_chain"):
    """
    Runs ai_classify and ai_extract on a parsed document.
    Returns: doc_type, confidence, extracted fields.
    Accepts optional ?domain_id= to use domain-specific labels/schemas.
    """
    _domain = _get_domain_schemas(domain_id)
    # Use domain classification labels when available, else fall back to module-level
    _labels_json = _domain.get("classification_labels") or CLASSIFICATION_LABELS
    labels_sql  = _labels_json.replace("'", "\\'")
    classify_instr = (
    ...
    parsed_sql = req.parsed_content.replace("'", "\\'")
    ...
    classify_query = f"""
        SELECT
            ai_classify(
                '{parsed_sql}',
                '{labels_sql}',
                MAP('instructions', '{classify_instr}')
            ):response[0]::STRING AS doc_type
    """
    classify_rows = run_sql(classify_query)
    doc_type = req.doc_type_hint or (classify_rows[0]["doc_type"] if classify_rows else "unknown")

    # Step 2: extract based on doc_type
    config = EXTRACTION_CONFIGS.get(doc_type, {
        "schema": DEFAULT_SCHEMA,
        "instructions": "Extract key business information from this supply chain document.",
    })
    schema_sql  = json.dumps(config["schema"]).replace("'", "\\'")
    extract_instr = config["instructions"].replace("'", "\\'")
    ...
    extract_query = f"""
        SELECT
            ai_extract(
                '{parsed_sql}',
                '{schema_sql}',
                MAP('instructions', '{extract_instr}')
            ) AS extracted
    """
    extract_rows = run_sql(extract_query)
    extracted_raw = extract_rows[0]["extracted"] if extract_rows else "{}"

    # Parse extracted JSON
    try:
        extracted = json.loads(extracted_raw) if isinstance(extracted_raw, str) else extracted_raw
        fields = extracted.get("response", extracted)
    except Exception:
        fields = {"raw": str(extracted_raw)}

    # Persist to gold table
    _sch_raw_persist = _domain.get("schema_raw") or SCH_RAW
    try:
        field_inserts = ", ".join(
            f"('{req.doc_id}', '{doc_type}', '{k}', '{str(v)}')"
            for k, v in fields.items() if v
        )
        if field_inserts:
            run_sql(f"""
                CREATE TABLE IF NOT EXISTS {CATALOG}.{_sch_raw_persist}.extracted_fields_interactive (
                    doc_id STRING, doc_type STRING, field_name STRING, field_value STRING
                )
            """)
            run_sql(f"""
                INSERT INTO {CATALOG}.{_sch_raw_persist}.extracted_fields_interactive
                (doc_id, doc_type, field_name, field_value)
                VALUES {field_inserts}
            """)
    except Exception:
    ...
    return {
        ...
        "schema_used": list(config["schema"].keys()),
    }


@router.get("/ontology")
async def get_ontology(entity_id: Optional[str] = None, doc_id: Optional[str] = None, domain_id: str = "supply_chain"):
    """
    Returns ontology entities and relationships.
    - If entity_id is given: returns all relationships for that entity.
    - If doc_id is given: returns entities sourced from that document.
    - If neither: returns all entities with their relationships.
    """
    _domain = _get_domain_schemas(domain_id)
    _ont = _domain["schema_ont"]
    if entity_id:
        entities = run_sql(f"""
            SELECT entity_id, entity_type, canonical_id, display_name, attributes, source
            FROM {CATALOG}.{_ont}.entities
            WHERE entity_id = '{entity_id}'
               OR canonical_id = '{entity_id}'
        """)
        relationships = run_sql(f"""
            SELECT r.subject_id, r.predicate, r.object_id, r.confidence,
                   es.entity_type AS subject_type, es.display_name AS subject_name,
                   eo.entity_type AS object_type, eo.display_name AS object_name
            FROM {CATALOG}.{_ont}.relationships r
            LEFT JOIN {CATALOG}.{_ont}.entities es ON es.entity_id = r.subject_id
            LEFT JOIN {CATALOG}.{_ont}.entities eo ON eo.entity_id = r.object_id
        ...
        relationships = run_sql(f"""
            ...
            FROM {CATALOG}.{_ont}.relationships
            ORDER BY subject_id
            LIMIT 500
        """)
...
@router.get("/recall-impact")
async def recall_impact(lot_number: str = "LOT-PP-240315", domain_id: str = "supply_chain"):
    """Returns full recall impact: impacted restaurants + financial breakdown."""
    _d = _get_domain_schemas(domain_id)
    _raw = _d["schema_raw"]; _ont = _d["schema_ont"]
    restaurants = run_sql(f"""
        SELECT
            r.restaurant_id, r.restaurant_name, r.state, r.region,
            dc.dc_name, inv.quantity_cases,
            ROUND(inv.quantity_cases * 30, 0) AS quantity_lbs,
            inv.delivery_date,
            ROUND(inv.quantity_cases * 450, 2) AS estimated_revenue_usd
        FROM {CATALOG}.{_raw}.inventory_distribution inv
        JOIN {CATALOG}.{_raw}.restaurants r ON inv.restaurant_id = r.restaurant_id
        JOIN {CATALOG}.{_raw}.distribution_centers dc ON r.dc_id = dc.dc_id
        WHERE inv.lot_number = '{lot_number}'
        ORDER BY inv.delivery_date, r.state
    """)

    financials = [
        {"component": "Contract Penalty — Temp Excursion Tier 2 (44–48°F)", "amount_usd": 15000, "source": "MSA-2024-TF-001 Sec 4.1"},
        {"component": "Recall Product Disposal (240 cases × $100)", "amount_usd": 24000, "source": "RCL-2024-0012"},
        {"component": "Expedited Microbiological Retesting", "amount_usd": 3500, "source": "QIR-2024-0047"},
        {"component": "Lost Restaurant Revenue (12 locations)", "amount_usd": 187000, "source": "Estimated"},
        {"component": "Carrier Liability Offset (Swift 20%)", "amount_usd": -4700, "source": "CSA-2024-SL-007"},
    ]
    total_gross = sum(abs(f["amount_usd"]) for f in financials)
    total_net   = sum(f["amount_usd"] for f in financials)

    recall = run_sql(f"""
        SELECT entity_id, attributes, display_name
        FROM {CATALOG}.{_ont}.entities
        WHERE entity_type = 'RecallEvent'
          AND attributes:lot_number::STRING = '{lot_number}'
        LIMIT 1
    """)

    return {
        "lot_number": lot_number,
        "recall": recall[0] if recall else {},
        "impacted_restaurants": restaurants,
...
async def supplier_risk(domain_id: str = "supply_chain"):
    """Returns all suppliers ranked by risk score with performance context."""
    _raw = _get_domain_schemas(domain_id)["schema_raw"]
    suppliers = run_sql(f"""
        SELECT
            s.supplier_id, s.supplier_name, s.region, s.risk_score, s.contract_id,
            CASE
...
async def incident_timeline(shipment_id: str = "SHP-20240315", domain_id: str = "supply_chain"):
    """Returns the full incident timeline for a shipment."""
    timeline = [
        {"event_time": "2024-03-15 06:23", "event": "Shipment Departs", "detail": f"{shipment_id} — Tyson Foods → ATL DC — Trailer TR-8821 — 480 cases Chicken Fillet", "severity": "info"},
        {"event_time": "2024-03-15 10:15", "event": "Temperature Excursion Begins", "detail": "Trailer TR-8821 temperature exceeds 40°F. Compressor HPO failure detected.", "severity": "critical"},
        {"event_time": "2024-03-15 10:45", "event": "Peak Temperature", "detail": "48.2°F recorded — 8.2°F above contractual limit of 40°F", "severity": "critical"},
        {"event_time": "2024-03-15 11:23", "event": "Temperature Restored", "detail": "Compressor restarted. Temperature returns to ≤40°F after 68-minute excursion.", "severity": "warning"},
        {"event_time": "2024-03-15 15:47", "event": "Arrives Atlanta DC", "detail": "Product placed on HOLD. Temperature log retrieved. QIR-2024-0047 opened.", "severity": "warning"},
        {"event_time": "2024-03-16 14:47", "event": "Conditional Release", "detail": "Dr. Marcus Williams approves conditional release. Lot PP-240315 distributed to Southeast restaurants.", "severity": "warning"},
        {"event_time": "2024-03-20 00:00", "event": "Recall Issued", "detail": "RCL-2024-0012 — Class II voluntary recall. LOT-PP-240315 recalled from 12 restaurants.", "severity": "critical"},
    ]

    # Try to enrich from structured data
    try:
        excursion = run_sql(f"""
            SELECT e.attributes
            FROM {CATALOG}.{SCH_ONT}.entities e
            WHERE e.entity_type = 'TemperatureExcursion'
              AND e.attributes:shipment_id::STRING = '{shipment_id}'
            LIMIT 1
        """)
        if excursion:
            attrs = json.loads(excursion[0]["attributes"]) if isinstance(excursion[0]["attributes"], str) else excursion[0]["attributes"]
            for event in timeline:
                if "Peak Temperature" in event["event"]:
                    event["ontology_entity"] = "TE-20240315-001"
                    event["peak_temp_f"] = attrs.get("peak_temp_f")
                    event["duration_min"] = attrs.get("duration_minutes")
    except Exception:
        pass
...
# ── Generic Domain Overview (non-supply-chain domains) ─────────────────────────
...
@router.get("/processing-log-stats")
async def get_processing_log_stats(domain_id: str = "supply_chain"):
    """
    Lightweight summary — total/success/new counts for dashboard badges.
    """
    _ensure_proc_log_table()
    try:
        rows = run_sql(f"""
            ...
            WHERE domain_id = '{domain_id}'
        """, timeout_secs=15) or []
        r = rows[0] if rows else {"total": 0, "processed": 0, "failed": 0}
        return {"ok": True, **r}
    except Exception as e:
        return {"ok": False, "total": 0, "processed": 0, "failed": 0, "error": str(e)}
...
async def unprocessed_docs(domain_id: str = "supply_chain"):
    """
    Returns the count and filenames of PDF/TXT files in the domain volume
    that have NOT yet been processed (not present in parsed_documents).
    Used to show a 'X new docs waiting' badge on the Run Pipeline button.
    """
    _d = _get_domain_schemas(domain_id)
    _raw   = _d["schema_raw"]
    _vol   = _d.get("volume_docs", "documents")
    ...
    try:
        ...
        try:
            for fi in w.files.list_directory_contents(volume_path):
                if fi.name and (fi.name.endswith(".pdf") or fi.name.endswith(".txt")):
                    all_files.append(fi.name)
        except Exception as e:
            return {"unprocessed_count": 0, "total_volume_files": 0,
                    "unprocessed_filenames": [], "warning": str(e)}

        if not all_files:
            return {"unprocessed_count": 0, "total_volume_files": 0,
                    "unprocessed_filenames": []}

        # Get already-processed base names (extension stripped) so that
        # doc.txt is not reported as new when doc.pdf has been processed.
        import os as _os
        processed_bases: set = set()
        try:
            rows = run_sql(f"SELECT doc_id FROM {CATALOG}.{_raw}.parsed_documents")
            processed_bases = {_os.path.splitext(r["doc_id"])[0] for r in rows}
        except Exception:
            pass  # table may not exist yet — all docs are new

        unprocessed = [f for f in all_files if _os.path.splitext(f)[0] not in processed_bases]
        return {
            "unprocessed_count":     len(unprocessed),
            "total_volume_files":    len(all_files),
            "processed_count":       len(processed_ids),
            "unprocessed_filenames": unprocessed[:50],  # cap for payload size
        }
    except Exception as e:
        return {"unprocessed_count": 0, "total_volume_files": 0,
                "unprocessed_filenames": [], "error": str(e)}
...
@router.post("/incidents/{incident_id}/link-doc")
async def link_document(incident_id: str, req: LinkDocRequest, domain_id: str = "supply_chain"):
    """Link a parsed document to an incident."""
    from datetime import datetime, timezone
    ts = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S")
    doc_path = req.document_path.replace("'", "''")
    doc_name = (req.document_name or req.document_path.split("/")[-1]).replace("'", "''")
    linked_by = req.linked_by.replace("'", "''")

    run_sql(f"""
        MERGE INTO {CATALOG}.{SCHEMA_PLATFORM}.incident_documents t
        USING (SELECT
            '{incident_id}' AS incident_id,
            '{domain_id}'   AS domain_id,
            '{doc_path}'    AS document_path,
            '{doc_name}'    AS document_name,
            TIMESTAMP('{ts}') AS linked_at,
            '{linked_by}'   AS linked_by
        ) s ON t.incident_id = s.incident_id AND t.document_path = s.document_path
        WHEN NOT MATCHED THEN INSERT *
    """, timeout_secs=30)
    return {"incident_id": incident_id, "document_path": req.document_path, "status": "linked"}


@router.delete("/incidents/{incident_id}/unlink-doc")
async def unlink_document(incident_id: str, document_path: str, domain_id: str = "supply_chain"):
    """Remove a document link from an incident."""
    doc_path = document_path.replace("'", "''")
    run_sql(f"""
        DELETE FROM {CATALOG}.{SCHEMA_PLATFORM}.incident_documents
        WHERE incident_id = '{incident_id}' AND document_path = '{doc_path}'
    """, timeout_secs=30)
    return {"incident_id": incident_id, "document_path": document_path, "status": "unlinked"}


# ── Smart Doc-to-Incident Matching ──────────────────────────────────────────

# Keyword patterns that signal relevance between doc types and incident types
_INCIDENT_DOC_AFFINITY: dict = {
    "product_recall": [
        "recall_notice", "quality_incident_report", "temperature_log",
        "bill_of_lading", "certificate_of_analysis", "email_chain",
        "inspection_report", "supplier_contract", "maintenance_report",
        "delivery_exception", "carrier_sla", "weather_report", "supplier_scorecard",
    ],
    "food_safety": [
        "quality_incident_report", "inspection_report", "certificate_of_analysis",
        "recall_notice", "temperature_log", "supplier_scorecard", "email_chain",
    ],
    "supplier_audit": [
        "supplier_contract", "supplier_scorecard", "certificate_of_analysis",
        "inspection_report", "quality_incident_report", "email_chain",
    ],
    "contract_dispute": [
        "supplier_contract", "carrier_sla", "supplier_scorecard",
        "bill_of_lading", "email_chain", "delivery_exception",
    ],
    "temperature_excursion": [
        "temperature_log", "maintenance_report", "bill_of_lading",
        "quality_incident_report", "weather_report", "carrier_sla",
    ],
}

@router.get("/suggest-docs")
async def suggest_docs_for_incident(incident_id: str, domain_id: str = "supply_chain"):
    """
    Returns a ranked list of documents that are most relevant to an incident,
    based on incident type affinity and keyword matching in document text.
    Already-linked documents are marked as such.
    """
    _d = _get_domain_schemas(domain_id)
    _raw = _d["schema_raw"]
    ...
    # Get already-linked doc names
    linked_rows = run_sql(f"""
        SELECT document_name, document_path
        FROM {CATALOG}.{SCHEMA_PLATFORM}.incident_documents
        WHERE incident_id = '{incident_id}'
    """, timeout_secs=30) or []
    linked_names = {r["document_name"] for r in linked_rows} | {r["document_path"] for r in linked_rows}

    # Get all docs
    all_docs = run_sql(f"""
        SELECT doc_id, filename, doc_type, LEFT(raw_text, 600) AS text_preview,
               LENGTH(raw_text) AS char_count
        FROM {CATALOG}.{_raw}.parsed_documents
        ORDER BY filename
    """, timeout_secs=30) or []
    ...
    priority_types = _INCIDENT_DOC_AFFINITY.get(inc_type, [])
    inc_keywords = set(
        w.lower() for w in (inc.get("title", "") + " " + inc.get("description", "")).split()
        if len(w) > 4
    )
    ...
    for doc in all_docs:
        ...
        already_linked = fname in linked_names or doc.get("doc_id", "") in linked_names

        # Score: affinity rank (lower = higher priority) + keyword hits
        doc_type = doc.get("doc_type", "unknown")
        affinity_rank = priority_types.index(doc_type) if doc_type in priority_types else len(priority_types)

        text = (doc.get("text_preview") or "").lower()
        kw_hits = sum(1 for kw in inc_keywords if kw in text)

        # relevance_note: human-readable reason
        if already_linked:
            relevance_note = "Already linked"
        elif doc_type in priority_types:
            relevance_note = f"Relevant doc type for {inc_type.replace('_', ' ')} incidents"
        elif kw_hits > 0:
            relevance_note = f"Contains {kw_hits} keyword(s) matching incident description"
        else:
            relevance_note = "Low direct relevance"
        ...
        results.append({
            ...
            "relevance_score": (len(priority_types) - affinity_rank) * 2 + kw_hits,
        })

    results.sort(key=lambda x: (-x["relevance_score"], x["filename"]))
    return {"incident_id": incident_id, "incident_type": inc_type, "suggestions": results}
