...
| Retrieval chunks | `{schema_vec}.document_chunks` (`chunk_id, doc_id, doc_type, chunk_position, chunk_to_retrieve, chunk_to_embed, source_uri`); queried at runtime by the SQL keyword shim `_vs_search` (no Vector Search index) | `notebooks/03_idp_pipeline.py` (produces table); `app/backend/docintel_routes.py` (`_vs_search`, ~297) |
